    ...
    getResData = () => {
        requests.getRestaurantInfo((res) => {
            if (res.hasOwnProperty("statusCode") && res.statusCode === 200) {
                ...
                let openTime = [];
                let sameDays = [];
                JSON.parse(res.data['openTime']).forEach(arr => {
                    openTime.push(_.sortBy(arr))
                })
                ...
                openTime.forEach(()=>{
                    openTime.forEach((day) => {
                        sameDays.push(_.remove(openTime, function (rDays) {
                                return _.isEqual(_.dropRight(day, 1), _.dropRight(rDays, 1));
                            })
                        )
                    })
                })
            ...
            <React.Fragment>
                ...
                <div className='resDetailsContainer IranSansLight'>
                    <div style={{minHeight: '150px', height: '150px'}}
                        ...
                        <img className={'res-details-logo'} src={this.state.logoLink}
                             style={{opacity: this.state.logoLink ? 1 : 0, transition: 'all 0.5s ease'}} alt=''/>
                    </div>

                    <section className={' w-100'} style={{
                        transition: 'all 0.2s ease',
                        opacity: this.state.resPhone ? 1 : 0,
                        display: this.state.resPhone.length ? 'block' : 'none'
                    }}>
                        ...
                        <div className={'mt-5 w-100 d-flex flex-row justify-content-around align-items-center'}>
                            ...
                            <div className={'res-details-icons'}
                                 style={{background: `url(${phoneImage})`, backgroundSize: 'cover'}}/>
                        </div>
                        <div className={'mt-3 w-100 d-flex flex-row justify-content-around align-items-center'}>
                            ...
                            <div className={'res-details-icons'}
                                 style={{background: `url(${locationImage})`, backgroundSize: 'cover'}}/>
                        </div>
                    ...
                    <section>
                        {this.state.resInstagram ?
                            <div>
                                <div className={'w-100 text-right pr-3 mt-5 IranSans'} style={{fontSize: '1.5rem'}}>لینک
                                    ارطباتی
                                </div>
                                <div className={'mt-3 w-100 d-flex flex-row justify-content-around align-items-center'}>
                                    <div className={'w-75 text-right'}>{this.state.resAddress}</div>
                                    <div className={'res-details-icons'}
                                         style={{background: `url(${instagramImage})`, backgroundSize: 'cover'}}/>
                                </div>
                            </div>
                            :
                            <div/>
                    ...
                    <section>
                        ...
                        <div className={'res-details-open-time-container d-flex flex-column align-items-start rtl'}>
                            <span
                                className={'open-time-texts'}>شنبه از {this.state.openTimes[0][0]} تا {this.state.openTimes[0][this.state.openTimes[0].length - 1]}</span>
                            <span
                                className={'open-time-texts'}>یک شنبه از {this.state.openTimes[1][0]} تا {this.state.openTimes[1][this.state.openTimes[1].length - 1]}</span>
                            <span
                                className={'open-time-texts'}>دو شنبه از {this.state.openTimes[2][0]} تا {this.state.openTimes[2][this.state.openTimes[2].length - 1]}</span>
                            <span
                                className={'open-time-texts'}>سه شنبه از {this.state.openTimes[3][0]} تا {this.state.openTimes[3][this.state.openTimes[3].length - 1]}</span>
                            <span
                                className={'open-time-texts'}>چهار شنبه از {this.state.openTimes[4][0]} تا {this.state.openTimes[4][this.state.openTimes[5].length - 1]}</span>
                            <span
                                className={'open-time-texts'}>پنج شنبه از {this.state.openTimes[5][0]} تا {this.state.openTimes[5][this.state.openTimes[5].length - 1]}</span>
                            <span
                                className={'open-time-texts'}>جمعه از {this.state.openTimes[6][0]} تا {this.state.openTimes[6][this.state.openTimes[6].length - 1]}</span>
